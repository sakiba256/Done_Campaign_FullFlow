...
class Drone {
  ...
  async stopCampaign(cData) {
    console.log('Need to Stop newly created Campaign');
    await this.page.waitForTimeout(4000); 
    for (const data of cData) {
      try {
        ...
        const stopButton= await this.page.locator('//div[@class="col-1 col-sm-1 col-md-1"]//div[2]').first();
        await stopButton.click();
        
        await this.page.waitForTimeout(2000);  
        const yesButton= await this.page.locator('//button[normalize-space()="Yes"]');
        await yesButton.click();
        await this.page.waitForTimeout(4000); 
        console.log('Stopped the Campaign Successfully');
        
      } catch (error) {
        console.error(`Error in catch Block creating Drone Campaign: ${error.message}`);
        const screenshotPath = `./Screenshots/stop-drone-campaign-${Date.now()}.png`;
        await this.page.screenshot({ path: screenshotPath });
        console.log(`Screenshot taken for Drone Campaign error: ${screenshotPath}`);
